...
export default function PersonnelManagement() {
  ...
  // Kurum listesini getir (atama seçimi için)
  ...
  const ROLE_BADGES: Record<string, { label: string; color: string; bg: string }> = {
    SUPER_ADMIN: { label: '👑 Süper Yönetici', color: '#dc2626', bg: '#fee2e2' },
    INSTITUTION_OFFICER: { label: '🏢 Kurum Yetkilisi', color: '#2563eb', bg: '#dbeafe' },
    CITIZEN: { label: '👤 Vatandaş / Üye', color: '#475569', bg: '#f1f5f9' },
  };

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
      {/* Top Banner & Search Section */}
      <div className="card" style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', color: '#fff', padding: '24px', border: '1px solid #334155' }}>
        ...
        <form onSubmit={handleSearch} style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
          <input
            ...
            style={{ flex: 1, minWidth: '260px', padding: '12px 16px', borderRadius: '10px', border: '1px solid #475569', background: '#1e293b', color: '#fff', fontSize: '14px' }}
          />
          <button
            type="submit"
            disabled={isSearching}
            style={{ padding: '12px 24px', borderRadius: '10px', background: '#3b82f6', color: '#fff', fontWeight: 700, border: 'none', cursor: isSearching ? 'not-allowed' : 'pointer', transition: 'all 0.2s' }}
          >
            {isSearching ? 'Aranıyor...' : '🔍 Vatandaş Ara & Ata'}
          </button>
        </form>

        {/* Search Results */}
        {searchResults.length > 0 && (
          <div style={{ marginTop: '20px', background: '#1e293b', padding: '16px', borderRadius: '12px', border: '1px solid #334155' }}>
            <h4 style={{ margin: '0 0 12px 0', fontSize: '14px', color: '#38bdf8' }}>Arama Sonuçları ({searchResults.length})</h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {searchResults.map((user) => (
                <div key={user.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#0f172a', padding: '12px 16px', borderRadius: '8px', border: '1px solid #334155' }}>
                  <div>
                    <div style={{ fontWeight: 700, color: '#fff' }}>{user.firstName} {user.lastName} <span style={{ fontSize: '12px', color: '#94a3b8' }}>({user.email})</span></div>
                    <div style={{ fontSize: '12px', marginTop: '4px', display: 'flex', gap: '8px', alignItems: 'center' }}>
                      <span style={{ padding: '2px 8px', borderRadius: '4px', background: ROLE_BADGES[user.role]?.bg || '#f1f5f9', color: ROLE_BADGES[user.role]?.color || '#475569', fontWeight: 700, fontSize: '11px' }}>
                        {ROLE_BADGES[user.role]?.label || user.role}
                      </span>
                      {user.institution && (
                        <span style={{ color: '#38bdf8' }}>📍 {user.institution.name}</span>
                      )}
                    </div>
                  </div>
                  <button
                    onClick={() => handleOpenAssignModal(user)}
                    style={{ padding: '8px 16px', borderRadius: '8px', background: '#2563eb', color: '#fff', fontWeight: 600, fontSize: '12px', border: 'none', cursor: 'pointer' }}
                  >
                    ⚙️ Yetkiyi / Kurumu Değiştir
                  </button>
                </div>
              ))}
            </div>
      ...
      <div className="card" style={{ padding: '0', overflowX: 'auto' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--color-border)' }}>
          <h4 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: 'var(--color-text)' }}>
            Aktif Kurum Personeli & Yöneticiler ({personnel.length})
          </h4>
        </div>
            ...
            <tbody>
              {personnel.map((p: any) => (
                <tr key={p.id} style={{ borderBottom: '1px solid var(--color-border)', fontSize: '14px', transition: 'background 0.15s' }}>
                  ...
                  <td style={{ padding: '16px 20px' }}>
                    <span style={{ padding: '4px 10px', borderRadius: '6px', background: ROLE_BADGES[p.role]?.bg || '#f1f5f9', color: ROLE_BADGES[p.role]?.color || '#475569', fontWeight: 700, fontSize: '12px' }}>
                  ...
                  <td style={{ padding: '16px 20px' }}>
                    {p.institution ? (
                      <div style={{ fontWeight: 600, color: 'var(--color-text)' }}>
                        🏢 {p.institution.name}
                        <div style={{ fontSize: '11px', color: 'var(--color-text-secondary)' }}>📍 {p.institution.city}</div>
                      </div>
                    ) : (
                  ...
                  <td style={{ padding: '16px 20px', textAlign: 'right', whiteSpace: 'nowrap' }}>
                    <button
                      ...
                      style={{ padding: '8px 14px', borderRadius: '8px', background: 'var(--color-surface-hover)', border: '1px solid var(--color-border)', color: 'var(--color-text)', fontWeight: 600, fontSize: '12px', cursor: 'pointer' }}
                    >
                      ⚙️ Düzenle
                    </button>
                  </td>
      ...
      {selectedUser && (
        <div style={{
          ...
          <div className="card" style={{ width: '100%', maxWidth: '500px', padding: '28px', background: 'var(--color-surface)', borderRadius: '16px', boxShadow: '0 20px 40px rgba(0,0,0,0.4)' }}>
            <h3 style={{ margin: '0 0 6px 0', fontSize: '18px', fontWeight: 700, color: 'var(--color-text)' }}>
              ⚙️ Rol ve Kurum Görevlendirmesi
            </h3>
            ...
            <form onSubmit={handleSaveRole} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '12px', paddingTop: '16px', borderTop: '1px solid var(--color-border)' }}>
                ...
                <button
                  type="submit"
                  disabled={isUpdating}
                  style={{ padding: '10px 22px', borderRadius: '8px', background: '#2563eb', color: '#fff', fontWeight: 700, border: 'none', cursor: isUpdating ? 'not-allowed' : 'pointer' }}
                >

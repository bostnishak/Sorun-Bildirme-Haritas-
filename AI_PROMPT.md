...
⚠️ BEYZA, GÜLSÜM, AHMET (EKİP ÜYELERİ) İÇİN KESİNLİKLE YASAK: Bu dosyalarda asla değişiklik yapma, migration çalıştırma veya şema güncelleme! Eğer geliştirdiğin özellik yeni bir veritabanı alanı veya modeli gerektiriyorsa, bunu kodun içine yorum satırı olarak not düş veya mevcut yapıyı kullan ama şemaya dokunma.
👑 İSHAK (PROJE LİDERİ) İÇİN SERBEST: İshak admin yetkisine sahiptir! İshak giriş yaptığında veya merge yaptığında bu dosyaları değiştirme, veritabanı şemasını güncelleme ve `prisma generate` çalıştırma yetkisi tamamen serbesttir.
...
🛡️ A) EĞER İSMİM "İshak" (Proje Lideri - Admin) İSE:

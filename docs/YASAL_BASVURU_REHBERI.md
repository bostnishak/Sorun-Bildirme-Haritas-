...
### 4. 🎤 Sesli İhbar & Canlı Konum Hukuki Statüsü (Biyometri & GPS)

Canlıya almadan önce en kritik konulardan biri olan **"Konum"** ve **"Mikrofon"** verileri için projenizde şu önlemler alınmıştır:
- **Ses Verisi (Biyometrik Risk Yoktur):** KVKK Madde 6 uyarınca ses kaydı (örn. mp3) tutmak biyometrik veri işleme statüsüne girebilir. Bu risk, projede mikrofondan alınan seslerin sunucuya gitmeden tarayıcı düzeyinde **anlık olarak metne çevrilmesi (Speech-to-Text)** yöntemiyle aşılmıştır. Sesler kaydedilmez, sadece dökülen metin loglanır. Bu durum açıkça Gizlilik ve KVKK sayfalarına eklenmiştir.
- **Canlı GPS Konumu:** Kullanıcının tarayıcı GPS (Geolocation API) onayı ile aldığı konum, ancak açık izin verildiğinde işlenir. Sunucu seviyesinde `nginx.conf` yapılandırmasındaki `Permissions-Policy` başlıkları bu izinlerin yalnızca uygulamanın kendisinde `(self)` çalışmasına olanak verecek şekilde düzenlenmiş ve yetkisiz dış erişimler tamamen engellenmiştir.
